...
CREATE TABLE Orders
(
    OrderID NUMBER(9) GENERATED ALWAYS AS IDENTITY START WITH 1 INCREMENT BY 1,
    ...
    CONSTRAINT FK_CUSTOMER_ID FOREIGN KEY (CustomerID) REFERENCES CUSTOMERS(CUSTOMER_ID),  
    CONSTRAINT FK_EMPLOYEE_ID FOREIGN KEY (EmployeeID) REFERENCES EMPLOYEES(EMPLOYEE_ID),  
    CONSTRAINT FK_SHIPPER_ID FOREIGN KEY (ShipVia) REFERENCES SHIPPERS(SHIPPER_ID)  
);

CREATE INDEX IDX_ORDERS_CUSTOMER_ID ON Orders(CustomerID);
CREATE INDEX IDX_ORDERS_EMPLOYEE_ID ON Orders(EmployeeID);
CREATE INDEX IDX_ORDERS_SHIPPER_ID ON Orders(ShipVia);
CREATE INDEX IDX_ORDERS_ORDER_DATE ON Orders(OrderDate);
CREATE INDEX IDX_ORDERS_SHIPPED_DATE ON Orders(ShippedDate);
CREATE INDEX IDX_ORDERS_SHIP_POSTAL_CODE ON Orders(ShipPostalCode);



COMMENT ON COLUMN Orders.OrderID IS 'Unique order number.';
COMMENT ON COLUMN Orders.CustomerID IS 'Same entry as in Customers table.';
COMMENT ON COLUMN Orders.EmployeeID IS 'Same entry as in Employees table.';
COMMENT ON COLUMN Orders.ShipVia IS 'Same as Shipper ID in Shippers table.';
COMMENT ON COLUMN Orders.ShipName IS 'Name of person or company to receive the shipment.';
COMMENT ON COLUMN Orders.ShipAdress IS 'Street address only -- no post-office box allowed.';
